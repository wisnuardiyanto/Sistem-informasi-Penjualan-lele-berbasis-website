...
if (isset($_POST['save'])) {
    $idpelanggan    = htmlentities(strip_tags(trim($_POST['idpelanggan'])));
    $namadepan      = htmlentities(strip_tags(trim($_POST['namadepan'])));
    $namabelakang   = htmlentities(strip_tags(trim($_POST['namabelakang'])));
    $alamat         = htmlentities(strip_tags(trim($_POST['alamat'])));
    $kodepos        = htmlentities(strip_tags(trim($_POST['kodepos'])));
    ...
    if (empty($namabelakang)) {
        $pesan_error .= "Nama Belakang belum diisi <br>";
    }
    if (empty($alamat)) {
    ...
    if (empty($notelp)) {
        $pesan_error .= "No. Telp belum diisi <br>";
    }
    if (empty($email)) {
    ...
    if ($pesan_error === "") {
        ...
        # code...
        $query = "update pelanggan set nama_depan='$namadepan',nama_belakang='$namabelakang',alamat='$alamat',kode_pos='$kodepos',no_telp='$notelp',email='$email',password='$password' where id_pelanggan='$idpelanggan'";
        if (mysqli_query($koneksi, $query)) {


            echo "<script> alert('Berhasil Tersimpan');</script>";
            $tampil_pelanggan = $koneksi->query("select * from pelanggan");
            echo "<script>location='../data-pelanggan.php';</script>";
        } else {
            # code...
            echo "<script> alert('Berhasil gagal Tersimpan');</script>";
        }
    }
} else {
    # code...
    $tampil_pelanggan = $koneksi->query("select id_pelanggan,nama_depan,nama_belakang,alamat,kode_pos,no_telp,email,password from pelanggan where id_pelanggan='$_GET[id]'");
    $data = $tampil_pelanggan->fetch_assoc();
            ...
            <div class="main-panel">
                ...
                ";
                                    ...
                                    <p class="card-description">
                                        Silahkan melakukan input data pelanggan
                                    </p>
                                    <form class="forms-sample" method="POST" enctype="multipart/form-data">
                                        <div class="form-group">
                                            <label for="idpelanggan">Id Pelanggan</label>
                                            <input type="text" class="form-control" id="idpelanggan" placeholder="idpelanggan" name="idpelanggan" value="<?= $id ?>">
                                        </div>
                                        <div class="form-group">
                                            <label for="namadepan">Nama Depan</label>
                                            <input type="text" class="form-control" id="namadepan" placeholder="namadepan" name="namadepan" value="<?= $namadepan ?>">
                                        </div>
                                        <div class="form-group">
                                            <label for="namabelakang">Nama Belakang</label>
                                            <input type="text" class="form-control" id="namabelakang" placeholder="namabelakang" name="namabelakang" value="<?= $namabelakang ?>">
                                        </div>
                                        <div class="form-group">
                                            <label for="alamat">Alamat</label>
                                            <input type="textarea" class="form-control" id="alamat" placeholder="alamat" name="alamat" value="<?= $alamat ?>">
                                        </div>
                                        <div class="form-group">
                                            <label for="kodepos">Kode Pos</label>
                                            <input type="text" class="form-control" id="kodepos" placeholder="kodepos" name="kodepos" value="<?= $kodepos ?>">
                                        </div>
                                        <div class="form-group">
                                            <label for="notelp">No. Telp</label>
                                            <input type="text" class="form-control" id="notelp" placeholder="notelp" name="notelp" value="<?= $notelp ?>">
                                        </div>
                                        <div class="form-group">
                                        ...
                                        <a class="btn btn-light me-2" href="http://localhost/penjualan-master/admin-dashboard/data-pelanggan.php" >Cancel</a>
                                    </form>
                                </div>
                            </div>
                        </div>
